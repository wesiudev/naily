...
import { PostImagesProps, ImageType } from "@/types";

export default function PostImages({ input, setInput }: PostImagesProps) {
  ...
  function handleImageUpload(img: File, idx: number) {
    setLoading(idx);
    const imageId = `image-${randId(20, "aA0")}`;
  ...
  function addMoreSlots(count = 8) {
    setEmptyRows((prev) => [
      ...prev,
      ...Array.from({ length: count }, () => ({ src: "", alt: "" })),
    ]);
  }
  ...
  return (
    <div className="w-full my-12">
      ...
      <div className="flex justify-between items-center mb-3">
        <div />
        <button
          onClick={() => addMoreSlots(8)}
          className="px-3 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-md text-white"
        >
          Dodaj 8 miejsc
        </button>
      </div>
