...
import { Post, ImageType } from "@/types";
...
export default function PostImages({
  ...
  function handleImageUpload(img: File, idx: number) {
    ...
    uploadBytes(imageRef, img).then(() =>
      getDownloadURL(imageRef).then((url) => {
        ...
        // Add to gallery array on upload
        setSelectedPost({
          ...selectedPost,
          images: [...(selectedPost.images || []), url],
        });
        setLoading(-1);
      })
    );
  ...
  async function handleMultipleImagesUpload(files: FileList) {
    ...
    try {
      ...
      setSelectedPost({
        ...selectedPost,
        images: [...(selectedPost.images || []), ...urls],
      });
    } catch (_e) {
      // no-op
    }
  ...
  return (
    <div className="w-full my-12">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="text-sm text-gray-300">Dodaj wiele zdjęć naraz</div>
        <div>
          <input
            id="multiInput"
            type="file"
            ...
            onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
              if (event.target.files) {
                handleMultipleImagesUpload(event.target.files);
          ...
          <label
            htmlFor="multiInput"
            className="px-3 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-md text-white cursor-pointer"
          >
            Wgraj wiele
          </label>
        </div>
      ...
      {(selectedPost.images || []).length > 0 && (
        <div className="mb-6">
          <h2 className="text-white text-sm font-medium mb-2">
            Galeria (kolejność między sekcjami)
          </h2>
          <div className="flex flex-wrap gap-3">
            {(selectedPost.images || []).map((src: string, i: number) => (
              <div key={i} className="relative">
                ...
                <button
                  ...
                  className="absolute -top-2 -right-2 bg-red-600 hover:bg-red-700 text-white rounded-full w-6 h-6 text-xs"
