...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      if (user?.customerId) {
        // User already has a customer ID, reuse it
        ...
        // Save customer ID to user document immediately
        const { updateUser } = await import("@/firebase");
        await updateUser(uid, { customerId: customerId });
      }
    } catch (error) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      ...(customerId ? { customer: customerId } : { customer_email: email }), // Use customer ID if available, otherwise use email
      line_items: [
        {
          price_data: {
            currency: "pln",
